...
namespace Microsoft.Xna.Framework
{
    ...
    public class PrimitiveSphere
    {
        ...
        {
            CreatePrimitiveSphere(subdivisionWidth, subdividsionHeight, scale, clockwise, invert, directionalFaces, null, 1f);
        }

        public PrimitiveSphere(int subdivisionWidth, int subdividsionHeight, float scale, bool clockwise, bool invert, bool directionalFaces, Texture2D heightMap, float dataScalar)
        {
            CreatePrimitiveSphere(subdivisionWidth, subdividsionHeight, scale, clockwise, invert, directionalFaces, heightMap, dataScalar);
        }

        public void CreatePrimitiveSphere(int subdivisionWidth, int subdividsionHeight, float scale, bool clockwise, bool invert, bool directionalFaces, Texture2D heightMap, float dataScalar)
        {
            ...
            for (int faceIndex = 0; faceIndex < 6; faceIndex++)
            {
                ...
                for (int y = 0; y < subdividsionHeight; y++)
                {
                    ...
                    for (int x = 0; x < subdivisionWidth; x++)
                    {
                        ...
                        if (hasHeightMapdata)
                            v0 = GetVertice(p0, faceIndex, directionalFaces, depth, uv0, heightdata, dataScalar, heightMap.Width, heightMap.Height);
                        else
                            v0 = GetVertice(p0, faceIndex, directionalFaces, depth, uv0);
        ...
        private VertexPositionNormalTexture GetVertice(Vector3 v, int faceIndex, bool directionalFaces, float depth, Vector2 uv, Color[] heightData, float dataScalar, int width, int height)
        {
            var v2 = Vector3.Transform(v, GetWorldFaceMatrix(faceIndex));
            var n = Vector3.Normalize(v2);
            //var suv = NormalTo2dSphericalUvCoordinates(n);
            ...
            v2 = n * (depth + mapDepth);
            return new VertexPositionNormalTexture(v2, FlatFaceOrDirectional(v2, faceIndex, directionalFaces, depth), uv);
        }
